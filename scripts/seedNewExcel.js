// Usage: node server/scripts/seedNewExcel.js "C:\Users\puska\Downloads\diet_chart_allergy_cleaned_no_legume_pulse_coconut_FINAL.xlsx"
// Deletes existing meals in MongoDB and seeds new meal data from all sheets in the Excel.
// Sets the "region" field on each meal to the name of the Excel sheet it came from.
...
function parseAllergens(val) {
  ...
  // Split on comma, slash, semicolon, or newlines
...
async function run() {
  ...
  if (!filePath) {
    console.error("Usage: node server/scripts/seedNewExcel.js <path-to-excel-file.xlsx>");
    process.exit(1);
  }
  ...
  console.log("Deleting all existing meals from the database...");
  ...
  console.log(`Successfully seeded ${insertResult.length} meals into MongoDB.`);
